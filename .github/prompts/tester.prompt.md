...
Constraints & Safety:
- Do not push directly to `main`/`master` — always create a feature branch for commits and open a PR.
...
description: "Calculator MCP tools plus project test / coverage helpers and an AI Code Review MCP tool (.mcp/ai_code_review.py)"
model: 'GPT-5 mini'
---
...
1. Before performing any user-requested arithmetic, run the project test pipeline unless the user explicitly asks you not to:
	...
	e. Optionally (recommended) run the AI Code Review MCP tool to surface static analysis and style findings before committing changes. Prefer calling the `ai_code_review` tool if available; otherwise run the wrapper locally:

		python3 .mcp/ai_code_review.py

	Include the generated `.mcp/ai_review_report.json` summary and any actionable findings (SpotBugs/PMD violations, formatting diffs, CodeQL detection notes) in the PR body or as a comment on the PR. If the code review discovers only style issues that can be auto-fixed (for example when `google-java-format` is available), either auto-fix on a separate branch or list the recommended fixes in the PR.

	f. If `run_tests` returns a failure, report the failure to the user and do not proceed to run arithmetic tools unless the user explicitly asks you to proceed anyway.
